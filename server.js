...
app.get('/api/decks/:id', async (req, res) => {
  try {
    const deck = await pool.query('SELECT * FROM decks WHERE deck_id = $1', [req.params.id]);
    const cards = await pool.query('SELECT * FROM cards WHERE deck_id = $1', [req.params.id]);
